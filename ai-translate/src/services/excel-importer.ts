...
import { TranslationEntry, databaseService } from './database';
...
export class ExcelImporter {
     ...
     * @returns 解析后的翻译条目数组
     */
    async parseExcelFile(file: File): Promise<TranslationEntry[]> {
     ...
     * @returns 导入的条目数量
     */
    async importExcelToDatabase(file: File): Promise<number> {
        try {
            // 初始化数据库（确保表已创建）
